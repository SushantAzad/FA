...
const TransactionHistory = () => {
  ...
  const summaryData = useMemo(() => {
    ...
    const totalVolume =
      completed?.reduce((sum, t) => sum + Math.abs(t?.amount), 0) * USD_TO_INR;
    const totalFees =
      filteredTransactions?.reduce((sum, t) => sum + t?.fees, 0) * USD_TO_INR;
    ...
    const purchaseAmount =
      purchases?.reduce((sum, t) => sum + t?.amount, 0) * USD_TO_INR;
    const saleAmount =
      sales?.reduce((sum, t) => sum + t?.amount, 0) * USD_TO_INR;
    const dividendAmount =
      dividends?.reduce((sum, t) => sum + t?.amount, 0) * USD_TO_INR;
    const feeAmount = fees?.reduce((sum, t) => sum + t?.amount, 0) * USD_TO_INR;

    const netProfitLoss =
      saleAmount + dividendAmount - purchaseAmount - feeAmount;
    const successRate =
    ...
    return {
      ...
      totalProperties: [...new Set(filteredTransactions.map((t) => t.property))]
        ?.length,
  ...
  useEffect(() => {
    ...
    if (filters?.property && filters?.property !== "all") {
      filtered = filtered?.filter(
        (t) =>
          t?.property?.toLowerCase()?.replace(/\s+/g, "-") === filters?.property
      );
    }
    ...
    if (filters?.amountMin) {
      filtered = filtered?.filter(
        (t) => Math.abs(t?.amount) >= parseFloat(filters?.amountMin)
      );
    }

    if (filters?.amountMax) {
      filtered = filtered?.filter(
        (t) => Math.abs(t?.amount) <= parseFloat(filters?.amountMax)
      );
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        ...
        <meta
          name="description"
          content="View and manage your complete transaction history with advanced filtering and export capabilities."
        />
      </Helmet>
      <NavigationBar />
      {/* Header */}
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground mt-1">
                Track and analyze all your investment activities with
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        {/* Summary Section */}
        <TransactionSummary summaryData={summaryData} onExport={handleExport} />
